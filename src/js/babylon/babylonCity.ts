...
const myScene = {
  ...
  createScene: async (canvas: HTMLCanvasElement, store: any) => {
    ...
    function highlightGrid(x: number, z: number) {
      // 重置所有未占用的格子顏色
      for (let i = 0; i < gridSize; i++) {
        for (let j = 0; j < gridSize; j++) {
          const cell: any = grid[i][j]
      ...
      const startX = Math.floor(
        (x + (gridSize / 2) * cellSize) / cellSize - actualWidth / 2
      )
      const startZ = Math.floor(
        (z + (gridSize / 2) * cellSize) / cellSize - actualDepth / 2
      )
    ...
    myScene.scene.onPointerObservable.add((pointerInfo) => {
      switch (pointerInfo.type) {
        ...
        case PointerEventTypes.POINTERMOVE:
          if (dragging) {
            const pickResult = myScene.scene?.pick(
              myScene.scene.pointerX,
              myScene.scene.pointerY
            )
            ...
            if (pickResult?.pickedMesh && pickResult.pickedMesh.metadata) {
              ...
              const gridX = Math.floor(
                (target.position.x + (gridSize / 2) * cellSize) / cellSize
              )
              const gridZ = Math.floor(
                (target.position.z + (gridSize / 2) * cellSize) / cellSize
              )
