...
import useFragmentShader from "../../js/shaders/fragment.glsl?raw"
// 首先定義完整的介面
interface IKTXConfig {
  ...
  wasmUASTCToRGB_A_UNORM: string | null
  wasmZSTDDecoder?: string | null
}
// 然後設置配置
const ktxConfig: Partial<IKTXConfig> = {
  // 指向本地的 JS 解碼器文件
...
// 應用配置
KhronosTextureContainer2.URLConfig = ktxConfig as any
type NullAble<T> = T | null
interface ChildDummyAll {
  [key: string]: any[] // 允许字符串键和数组值
}
...
const togglePyramids = (
  ...
  const animate = (pyramid: any, targetY: any) => {
    animatePyramid(
      scene,
      pyramid,
      "position",
...
const myScene = {
  engine: null as NullAble<Engine>,
  scene: null as Scene | null,
  camera: null as ArcRotateCamera | null,
  canvas: null,
  model: null as NullAble<AbstractMesh>,
  vuePbrMaterial: null as PBRMaterial | null,
  vueConeTop: null as Mesh | null,
  vueConeBottom: null as Mesh | null,
  vueHeight: 1,
  vueSize: 1,
  createScene: async (canvas: HTMLCanvasElement, store: any) => {
    ...
    myScene.camera = new ArcRotateCamera(
      "camera",
      Math.PI / 2,
      Math.PI / 2,
      4,
    ...
    if (canvas) {
      ...
      myScene.engine.runRenderLoop(() => {
        if (myScene.scene) myScene.scene.render()
      })
  ...
  createAnimations: () => {
    ...
    animations.push(
      myScene.createAnimation(
        "targetAnim",
        "target",
        80,
        Animation.ANIMATIONTYPE_VECTOR3,
        myScene.camera?.target,
        new Vector3(0, 0, 0)
      )
    )
  ...
  appModelReset: () => {
    myScene.scene?.beginDirectAnimation(
      myScene.camera,
      myScene.createAnimations(),
      0,
      100,
      false
    )
