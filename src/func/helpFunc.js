...
//號碼長度限制並過濾消毒(XSRF)
const inputFilter = (e, number) => {
    return new Promise((resolve) => {
        ;
        e.target.value = e.target.value.replace(/[^\d]/g, ""); //不是數字會取代為空值
        if (e.target.value.length > number) {
            ;
            e.target.value = e.target.value.slice(0, number);
        }
        resolve(); // 執行成功時 resolve
    });
};
// 大圖載入判斷
const imgLoad = (currentClass) => {
    let avatar = "";
    let imgUrl = "";
    let modalElement = document.querySelector(`.${currentClass}`);
    if (currentClass) {
...
//# sourceMappingURL=helpFunc.js.map